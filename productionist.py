...
class Productionist(object):
    ...
    def __init__(self, content_bundle_name, content_bundle_directory, probabilistic_mode=False,
        ...
        # Next, load the trie file at the specified location; this file contains a data structure
    ...
    def _load_trie(self, trie_file_location):
        ...
        try:
            ...
            raise Exception(
                "Cannot load trie -- there is no trie file located at '{filepath}'".format(
                    filepath=trie_file_location
                )
            )
        return trie
    ...
    def _score_candidate_recipe(self, recipe):
        ...
        """
        trie_key_of_candidate_recipe_grammar_path = self.trie.restore_key(recipe.path_trie_key)
        path_string = self.trie.restore_key(trie_key_of_candidate_recipe_grammar_path)
        if path_string:
            path = [self.grammar.production_rules[int(i)] for i in path_string.split(',')]
        else:
            # If there's no path string, that means that the selected path is one that doesn't pass
            # through any symbols with tags; in this case, Productionist can just select between
            # production rules that are not semantically meaningful until it's ground out into a
            # terminal expansion
            path = []
    ...
    def _follow_recipe(self, recipe):
        ...
        # Unpack the trie key to build a path expressed as a list of semantically meaningful
        # production rules
        path_string = self.trie.restore_key(recipe.path_trie_key)
        if path_string:
            path_for_this_recipe = [self.grammar.production_rules[int(i)] for i in path_string.split(',')]
        else:
            # If there's no path string, that means that the selected path is one that doesn't pass
            # through any symbols with tags; in this case, Productionist can just randomly select
            # production rules that are not semantically meaningful until it's ground out into a
            # terminal expansion
            path_for_this_recipe = []
        # Keep the remaining named rules handy, since we'll be consuming this as we proceed
        self.remaining_path = path_for_this_recipe
...
class Recipe(object):
    ...
    def __init__(self, recipe_id, expressible_meaning, path_trie_key):
        ...
        self.path_trie_key = path_trie_key
...
if __name__ == "__main__":
    ...
    HAVE_REPETITIONS_FILE_PERSIST_ACROSS_RUNTIME_INSTANCES = False
    REPETITION_PENALTY_MULTIPLIER = 0.033  # Initially 30 times less likely to be used after first usage
    REPETITION_PENALTY_RECOVERY_RATE = 1.2  # Sheds 15% of its current penalty after each non-usage instance
    # Note: to see how many turns it will take for a symbol's repetition penalty to fully refresh (to a 1.0 value),
    # repeatedly multiple a value initialized to REPETITION_PENALTY_MULTIPLIER by REPETITION_PENALTY_RECOVERY_RATE
    # until the value reaches 1.0 (e.g., for REPETITION_PENALTY_MULTIPLIER=0.033 and REPETITION_PENALTY_RECOVERY_RATE
    # =1.2, it takes 19 turns for a symbol to fully refresh)
    # Parse the command-line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "content_bundle_name",
        help="the name of the content bundle that is to be operated over (i.e., 'robotDemo' to use files " +
             "generated by Reductionist that are named 'robotDemo.grammar', 'robotDemo.trie', and 'robotDemo.meanings'"
    )
    parser.add_argument(
        "content_bundle_dir",
        help="the full filepath to the bundle of content files that have been generated by Reductionist"
    )
    parser.add_argument(
        "--n",
        help="the number of generated outputs that are requested (default: 1)",
        type=int,
        default=1
    )
    parser.add_argument(
        "--symbol",
        help="the name of the nonterminal symbol that will be expanded to generate n examples; passing " +
             "an argument for this will cancel any arguments passed for 'rule', 'must_have', " +
             "'must_not_have', and 'scoring_metric'."
    )
    parser.add_argument(
        "--rule",
        help="the definition of the production rule ('[rule name] --> [rule body]') that will be " +
    ...
    parser.add_argument(
        "--must_have",
        help="tags that must be attached to the generated output; " +
             "tags should be formatted [tagset]:[tag],[tagset:tag],[tagset:tag]"
    )
    parser.add_argument(
        "--must_not_have",
        help="tags that must *not* be attached to the generated output; " +
             "tags should be formatted [tagset]:[tag],[tagset:tag],[tagset:tag]"
    )
    parser.add_argument(
        "--scoring_metric",
        help="a scoring metric for any other tags, indicating how desirable (or undesirable) they are currently; " +
             "tags should be formatted [tagset]:[tag]*[weight],[tagset:tag]*[weight],[tagset:tag]*[weight]"
    )
    parser.add_argument(
        "--nonprobabilistic",
        help="whether to disengage probabilistic mode (flag argument); if probabilistic mode is not engaged, the " +
    ...
    parser.add_argument(
        "--repetition_penalty",
        help="whether to engage repetition-penalty mode (flag argument); when repetition-penalty mode is turned " +
             "on, Productionist will be less likely to take paths in the grammar that were recently taken.",
        action="store_true"
    )
    parser.add_argument(
        "--terse",
        help="whether to engage terse mode (flag argument); when terse mode is engaged, shorter outputs (in " +
             "terms of number of characters) will be prioritized.",
        action="store_true"
    )
    parser.add_argument(
        "--test",
        help="whether to engage test mode (flag argument); when test mode is engaged, the system forms a random " +
             "content request and attempts to satisfy it.",
        action="store_true"
    )
    parser.add_argument(
        "--seed",
        help="an integer seed for the pseudorandom number generator that Productionist uses",
        type=int
    )
    parser.add_argument(
        "--verbosity",
        help="how verbose Productionist's debug text should be (0=no debug text, 1=more debug text, 2=most debug text)",
        type=int
    )
    ...
    productionist = Productionist(
        content_bundle_name=args.content_bundle_name,
        content_bundle_directory=args.content_bundle_dir,
        probabilistic_mode=not args.nonprobabilistic,
        repetition_penalty_mode=args.repetition_penalty,
        terse_mode=args.terse,
        verbosity=args.verbosity
    )
    if args.symbol:  # Expand a particular nonterminal symbol
        outputs = [
            productionist.furnish_example_terminal_expansion_of_nonterminal_symbol(nonterminal_symbol_name=args.symbol)
            for _ in xrange(args.n)
        ]
    elif args.rule:  # Execute a particular rule
        outputs = [
            productionist.furnish_example_terminal_result_of_executing_production_rule(
                production_rule_definition=args.rule
            )
            for _ in xrange(args.n)
        ]
    else:  # Build a content request by parsing the relevant command-line arguments
        if not args.test:
            must_have = set(args.must_have.split(',')) if args.must_have else set()
            must_not_have = set(args.must_not_have.split(',')) if args.must_not_have else set()
            scoring_metric = []
            if args.scoring_metric:
                for tag_and_weight in args.scoring_metric.split(','):
                    tag, weight = tag_and_weight.split('*')
                    weight = int(weight)
                    scoring_metric.append((tag, weight))
        else:  # Form a content request that solicits a randomly selected tag
            all_tags = set()
            for s in productionist.grammar.nonterminal_symbols:
                all_tags |= set(s.tags)
            must_have = {random.choice(list(all_tags))}
            must_not_have = set()
            scoring_metric = []
        content_request = ContentRequest(
            must_have=must_have, must_not_have=must_not_have, scoring_metric=scoring_metric
        )
        if args.verbosity > 0:
            print (
                "\n-- Attempting to fulfill the following content request:\n" +
                "\n\tMust have: {}".format(
                    ', '.join(content_request.must_have) if content_request.must_have else 'N/A'
                ) +
                "\n\tMust not have: {}".format(
                    ', '.join(content_request.must_not_have) if content_request.must_not_have else 'N/A'
                ) +
                "\n\tScoring metric: {}".format(
                    ', '.join(str(t) for t in scoring_metric) if scoring_metric else 'N/A'
                )
            )
        # Fulfill the content request to generate an output (as an object of the class Output, defined above)
        outputs = [productionist.fulfill_content_request(content_request=content_request) for _ in xrange(args.n)]
